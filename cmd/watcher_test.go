...
func (m *MockLogger) With(keyvals ...interface{}) cometlog.Logger {
	args := m.Called(keyvals)
	return args.Get(0).(cometlog.Logger)
}
...
func TestReconcileSentries(t *testing.T) {
	...
	err := sw.reconcileSentries(ctx, 1024, nil)
	require.NoError(t, err)
...
func TestReconcileSentries_WithLabelSelector(t *testing.T) {
	...
	selector := labels.SelectorFromSet(labels.Set{"environment": "production"})

	// Run reconcileSentries
	err = sw.reconcileSentries(ctx, 1024, selector)
	require.NoError(t, err)
...
func TestReconcileSentries_WithParseToLabelSelector(t *testing.T) {
	...
	// Create a CSV label string and parse it to a label selector
	csvLabels := "chain=osmosis,network=mainnet"
	selector, err := metav1.ParseToLabelSelector(csvLabels)
	require.NoError(t, err)

	// Run reconcileSentries with the selector
	err = sw.reconcileSentries(ctx, 1024, labels.SelectorFromSet(selector.MatchLabels))
	require.NoError(t, err)
